...
{
	{NULL,NULL,TASK_NORMAL,0,-1,TRUE,0},
	{NULL,NULL,TASK_PAUSE,0,-1,FALSE,0},
	{NULL,NULL,TASK_MOVETOPOSITION,0,-1,TRUE,0},
	{NULL,NULL,TASK_FOLLOWUSER,0,-1,TRUE,0},
	{NULL,NULL,TASK_WAITFORGO,0,-1,TRUE,0},
	{NULL,NULL,TASK_PICKUPITEM,0,-1,TRUE,0},
	{NULL,NULL,TASK_CAMP,0,-1,TRUE,0},
	{NULL,NULL,TASK_PLANTBOMB,0,-1,FALSE,0},
	{NULL,NULL,TASK_DEFUSEBOMB,0,-1,FALSE,0},
	{NULL,NULL,TASK_ATTACK,0,-1,FALSE,0},
	{NULL,NULL,TASK_ENEMYHUNT,0,-1,FALSE,0},
	{NULL,NULL,TASK_SEEKCOVER,0,-1,FALSE,0},
	{NULL,NULL,TASK_THROWHEGRENADE,0,-1,FALSE,0},
	{NULL,NULL,TASK_THROWFLASHBANG,0,-1,FALSE,0},
	{NULL,NULL,TASK_SHOOTBREAKABLE,0,-1,FALSE,0},
	{NULL,NULL,TASK_HIDE,0,-1,FALSE,0},
	{NULL,NULL,TASK_BLINDED,0,-1,FALSE,0},
	{NULL,NULL,TASK_SPRAYLOGO,0,-1,FALSE,0}
};
...
//	pBot->bInMessageLoop=FALSE;	
	...
	bottask_t TempTask = {NULL,NULL,TASK_NORMAL,TASKPRI_NORMAL,-1,0,TRUE};
				...
				if(fKills > 1 && g_fTimeRoundMid > gpGlobals->time && g_cKillHistory > 0)
				{
					...
					if(pBot->fBaseAgressionLevel < fKills)
					{
						...
						bottask_t TempTask = {NULL,NULL,TASK_MOVETOPOSITION,TASKPRI_MOVETOPOSITION,
							BotFindDefendWaypoint(pBot,paths[iWaypoint]->origin),0,TRUE};
						BotPushTask(pBot,&TempTask);
					}
				}
...
//		fp=fopen("bot.txt","a"); fprintf(fp, "MSG_CS_BUY (Pre): %s - iBuyCount=%d\n",pBot->name,pBot->iBuyCount); fclose(fp);
	...
	case MSG_CS_RADIO_SELECT:
		snprintf(szMenuSelect, sizeof(szMenuSelect), "%d", pBot->iRadioSelect);
		szMenuSelect[1]=0;
		...
		if (BotEntityIsVisible( pBot, pPlayer->v.origin))
		{
			...
			if((pBot->pBotUser == NULL) && (RANDOM_LONG(0,100)<50))
			{
				...
				if(iNumFollowers<g_iMaxNumFollow)
				{
					...
					bottask_t TempTask = {NULL,NULL,TASK_FOLLOWUSER,TASKPRI_FOLLOWUSER,-1,0,TRUE};
					BotPushTask(pBot,&TempTask);
				}
				else
		...
		if(pBot->pBotUser==NULL)
		{
			if(pBot->pBotEnemy==NULL)
			{
				...
				bottask_t TempTask = {NULL,NULL,TASK_MOVETOPOSITION,
					TASKPRI_MOVETOPOSITION,-1,0,TRUE};
				BotPushTask(pBot,&TempTask);
			}
			else
				BotPlayRadioMessage(pBot,RADIO_NEGATIVE);
...
//						if((iTeam==0) && (pBot->bot_weapons & (1<<CS_WEAPON_C4)))
						{
							...
							bottask_t TempTask = {NULL,NULL,TASK_PLANTBOMB,TASKPRI_PLANTBOMB,-1,0,FALSE};
							BotPushTask(pBot,&TempTask);
	...
	case TASK_PICKUPITEM:
		...
		case PICKUP_PLANTED_C4:
			...
			if((pBot->iTeam == 1) && (f_item_distance<50))
			{
				...
				BotPushMessageQueue(pBot,MSG_CS_TEAMSAY);
				bottask_t TempTask = {NULL,NULL,TASK_DEFUSEBOMB,TASKPRI_DEFUSEBOMB,-1,0,FALSE};
				BotPushTask(pBot,&TempTask);
